...
&nbsp;       即可启用cudnn加速了！
&nbsp;       cudnn其实对于模型的精度并没有多少提升，但是它极大的提高了我们模型运行的速度，在调整batch和workers发挥的作用尤其明显。例如，在batch为16，workers为4的情况下，模型运行的平均时间由66.44s降到了51.26s，将近77%的时间缩减。而在batch和workers都为默认值的初始状态下，时间仅仅从404.15s降到了393.50s，提速效果也就没那么明显了，总的来说，cudnn可以显著提高当batch和workers数量提高时模型的运行速度
...
```
&nbsp;      这是最终呈现的图片
&nbsp;      注：这里g代表gpu，w代表worker的数量，b代表batch的数量，cudnn表示使用了cudnn
![https://www.helloimg.com/i/2024/10/15/670e1e43d9a51.png](https://www.helloimg.com/i/2024/10/15/670e1e43d9a51.png)
## Ⅴ.实验过程中遇到的问题及解决方案
&nbsp;      
### ①.虚拟机平台选择
